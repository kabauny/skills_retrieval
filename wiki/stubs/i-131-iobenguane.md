---
title: "I-131 iobenguane"
entity_type: drug
aliases: ["Azedra"]
auto_generated: true
stub_target: entities
auto_source: "[[what-are-some-treatment-options-for-paraganglioma-that-is-not-resectable]]"
auto_date: 2026-04-26
tags: [auto-generated]
---

# I-131 iobenguane

> ⚠️ **Auto-generated stub** from a UI-driven grounded search. Verify and expand before relying on this for clinical decisions. The agent ingest workflow can promote this stub to a full entity/concept page.

## Brief

A high-specific-activity radiopharmaceutical agent (marketed as Azedra) approved for patients with unresectable, locally advanced, or metastatic paragangliomas that show uptake on an MIBG scan.

## Why this matters

It provides targeted radiation delivery directly to paraganglioma cells that express specific receptors, representing a crucial radiopharmaceutical therapy option for this rare cancer.

## Sources

- [[what-are-some-treatment-options-for-paraganglioma-that-is-not-resectable]] — origin search; see for full grounded citations and search queries
